        ...
        if(sender instanceof Player){
            ...
            if(args[0].equalsIgnoreCase("disband") && args.length == 2){
                if(args[1].equalsIgnoreCase("abort")){
                    ChatSender.sendPlayerMessage("Aborting...", true, sender, prefix);
                }else{
                    String divisionName = args[1];
                    divisionsManager.disbandDivision(divisionName);
                    ChatSender.sendPlayerMessage("You have disbanded your division!", true, sender, prefix);
                }
                ...
                if(plugin.hasBeenInvitedRecently(player.getUniqueId())){
                    if(!args[1].equalsIgnoreCase("abort")){
                        ...
                        for(UUID uuid : division.getPlayers()){
                            ...
                            if(member.getPlayer().isOnline()){
                                ...
                                ChatSender.sendPlayerMessage("The player &e" + player.getName() + "&f has joined the division", true, onlinePlayer, prefix);
                            }
                        }
                    ...
                    ChatSender.sendPlayerMessage("You haven't been invited to any divisions recently!", true, sender, prefix);
                }
            }
        }
